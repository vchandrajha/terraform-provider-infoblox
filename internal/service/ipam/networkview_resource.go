...
var (
	_ resource.Resource                     = &NetworkviewResource{}
	_ resource.ResourceWithValidateConfig   = &NetworkviewResource{}
	_ resource.ResourceWithConfigure        = &NetworkviewResource{}
	_ resource.ResourceWithImportState      = &NetworkviewResource{}
	_ resource.ResourceWithIdentity         = &NetworkviewResource{}
	_ resource.ResourceWithModifyPlan       = &NetworkviewResource{}
)
...
type NetworkviewResource struct {
	...
	service coresvc.NetworkviewService
}
...
func (r *NetworkviewResource) IdentitySchema(_ context.Context, _ resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
...
func (r *NetworkviewResource) Schema(_ context.Context, _ resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Manages an Infoblox Networkview across NIOS and UDDI backends.",
		Attributes:          NetworkviewResourceSchemaAttributes,
	}
}

func (r *NetworkviewResource) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	r.service = coresvc.NewNetworkviewService(r.backend, client.NIOS, client.UDDI)
}

func (r *NetworkviewResource) retryPolicy(op retry.Operation) retry.Policy {
	return retry.For[coremodel.Networkview](r.backend, op)
}

func (r *NetworkviewResource) ValidateConfig(ctx context.Context, req resource.ValidateConfigRequest, resp *resource.ValidateConfigResponse) {
...
func (r *NetworkviewResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if r.backend == core.BackendNIOS {
		nios := flex.ExpandNestedObject[NIOSNetworkviewModel](ctx, data.NIOS, &resp.Diagnostics)
		if nios == nil {
			nios = &NIOSNetworkviewModel{}
		}
		nios.ExtAttrs = flex.SetInternalID(ctx, nios.ExtAttrs, &resp.Diagnostics)
		if resp.Diagnostics.HasError() {
			return
		}
		data.NIOS = flex.FlattenNestedObject(ctx, nios, NIOSNetworkviewAttrTypes, &resp.Diagnostics)
	}
	...
	err := retry.Do(ctx, r.retryPolicy(retry.OpCreate), func(ctx context.Context) (int, error) {
		...
		apiResp, httpResp, apiErr = r.service.Create(ctx, obj, &core.Options{
			ReturnFields: NetworkviewReturnFields,
			Inherit:      NetworkviewInheritanceType,
		})
		if httpResp != nil {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to create Networkview: %s", err))
		return
	}
...
func (r *NetworkviewResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read Networkview: %s", err))
		return
	}
	...
	if r.backend == core.BackendNIOS && associateInternalId == nil && apiResp.NIOS != nil {
		...
		stateNIOS := flex.ExpandNestedObject[NIOSNetworkviewModel](ctx, data.NIOS, &resp.Diagnostics)
		if stateNIOS == nil || stateNIOS.ExtAttrsAll.IsNull() || stateNIOS.ExtAttrsAll.IsUnknown() {
			resp.Diagnostics.AddError(
				"Missing Internal ID",
				"Unable to read Networkview because the internal ID (from ext_attrs_all) is missing or invalid.",
			)
			return
		}
...
func (r *NetworkviewResource) ReadByExtAttrs(ctx context.Context, data *NetworkviewModel, resp *resource.ReadResponse) bool {
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to search Networkview by extattrs: %s", err))
		return true
	}
...
func (r *NetworkviewResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if r.backend == core.BackendNIOS {
		...
		planNIOS := flex.ExpandNestedObject[NIOSNetworkviewModel](ctx, data.NIOS, &resp.Diagnostics)
		stateNIOS := flex.ExpandNestedObject[NIOSNetworkviewModel](ctx, stateNIOSObj, &resp.Diagnostics)
		if planNIOS == nil {
			planNIOS = &NIOSNetworkviewModel{}
		}
		...
		data.NIOS = flex.FlattenNestedObject(ctx, planNIOS, NIOSNetworkviewAttrTypes, &resp.Diagnostics)
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update Networkview: %s", err))
		return
	}

	// Restore the plan ext_attrs (without inherited EAs) so Flatten preserves the user's input
	if r.backend == core.BackendNIOS {
		niosObj := flex.ExpandNestedObject[NIOSNetworkviewModel](ctx, data.NIOS, &resp.Diagnostics)
		if niosObj != nil {
			niosObj.ExtAttrs = planExtAttrs
			data.NIOS = flex.FlattenNestedObject(ctx, niosObj, NIOSNetworkviewAttrTypes, &resp.Diagnostics)
			if resp.Diagnostics.HasError() {
				return
			}
		}
...
func (r *NetworkviewResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	if err != nil {
		...
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete Networkview: %s", err))
	}
}

// ModifyPlan marks tags_all as unknown when tags changes, overriding UseStateForUnknown
// so the framework accepts the new tags_all value returned by the API after apply.
func (r *NetworkviewResource) ModifyPlan(ctx context.Context, req resource.ModifyPlanRequest, resp *resource.ModifyPlanResponse) {
	// Only relevant for UDDI on update (state and plan both exist)
	if req.State.Raw.IsNull() || req.Plan.Raw.IsNull() {
		return
	}

	var stateTags, planTags types.Map
	req.State.GetAttribute(ctx, path.Root("uddi").AtName("tags"), &stateTags)
	req.Plan.GetAttribute(ctx, path.Root("uddi").AtName("tags"), &planTags)

	if !stateTags.Equal(planTags) {
		resp.Plan.SetAttribute(ctx, path.Root("uddi").AtName("tags_all"), types.MapUnknown(types.StringType))
	}
}
