...
func (m *NetworkviewCloudInfoModel) Expand(ctx context.Context, diags *diag.Diagnostics) *niosipam.NetworkviewCloudInfo {
	...
	to := &niosipam.NetworkviewCloudInfo{
		...
		DelegatedRoot:   flex.ExpandStringPointer(m.DelegatedRoot),
		OwnedByAdaptor:  flex.ExpandBoolPointer(m.OwnedByAdaptor),
		Usage:           flex.ExpandStringPointer(m.Usage),
		Tenant:          flex.ExpandStringPointer(m.Tenant),
		MgmtPlatform:    flex.ExpandStringPointer(m.MgmtPlatform),
		AuthorityType:   nil, // read-only in NIOS, never send on create/update
	}
	return to
}
