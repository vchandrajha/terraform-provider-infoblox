...
var _ datasource.DataSourceWithValidateConfig = &NetworkviewDataSource{}
var _ datasource.DataSourceWithConfigure = &NetworkviewDataSource{}
...
type NetworkviewDataSourceModel struct {
...
func (m *NetworkviewDataSourceModel) FlattenResults(ctx context.Context, from []*coremodel.Networkview, diags *diag.Diagnostics) {
	if len(from) == 0 {
		m.Results = types.ListNull(types.ObjectType{AttrTypes: NetworkviewAttrTypes})
		return
	}
	...
	for _, obj := range from {
		...
		objValue, d := types.ObjectValueFrom(ctx, NetworkviewAttrTypes, model)
		diags.Append(d...)
		elements = append(elements, objValue)
	}
	list, d := types.ListValue(types.ObjectType{AttrTypes: NetworkviewAttrTypes}, elements)
	diags.Append(d...)
	m.Results = list
}

func (d *NetworkviewDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Retrieves information about existing Infoblox Networkview across NIOS and UDDI backends.",
		Attributes: map[string]schema.Attribute{
			...
			"results": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: utils.DataSourceResultAttributes(NetworkviewResourceSchemaAttributes),
				},
				Computed: true,
			},
...
func (d *NetworkviewDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.service = coresvc.NewNetworkviewService(d.backend, client.NIOS, client.UDDI)
}

func (d *NetworkviewDataSource) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
	var data NetworkviewDataSourceModel
...
func (d *NetworkviewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var allResults []*coremodel.Networkview
	var err error

	switch d.backend {
	case core.BackendNIOS:
		allResults, err = core.ReadAllPagesNIOS(func(pageID string) ([]*coremodel.Networkview, string, error) {
			opts.PageID = pageID
			recs, _, nextPageID, e := d.service.List(ctx, opts)
			return recs, nextPageID, e
		})
	case core.BackendUDDI:
		allResults, err = core.ReadAllPagesUDDI(func(offset, limit int32) ([]*coremodel.Networkview, error) {
			opts.Offset = offset
	...
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list Networkview records: %s", err))
		return
	}
