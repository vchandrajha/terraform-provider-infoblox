...
var (
	_ list.ListResource              = &NetworkviewList{}
	_ list.ListResourceWithConfigure = &NetworkviewList{}
)
...
func (l *NetworkviewList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewNetworkviewService(l.backend, client.NIOS, client.UDDI)
}

func (l *NetworkviewList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox Networkview from the configured backend (NIOS or UDDI).",
...
func (d *NetworkviewList) ValidateConfig(ctx context.Context, req datasource.ValidateConfigRequest, resp *datasource.ValidateConfigResponse) {
...
func (l *NetworkviewList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	...
	var records []*coremodel.Networkview
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.Networkview, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.Networkview, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list Networkview records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
